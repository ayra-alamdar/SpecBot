...
const ParallelCodePage = ({ user }) => {
    ...
    let loops = Object.values(extractedData);

    if (sortByDifference) {
        loops.sort((a, b) => {
            const diffA = Math.abs((a.Parallelized_Loop?.length || 0) - (a.Tiled_Loop?.length || 0));
            const diffB = Math.abs((b.Parallelized_Loop?.length || 0) - (b.Tiled_Loop?.length || 0));
            return diffB - diffA;
        });
    }
            ...
            <div className="content-container">
                <div className="loop-section">
                    {loops.map((loop, index) => {
                        if ((filter === "parallelized" && loop.Parallelized_Loop === "Not Parallelized") ||
                            (filter === "tiled" && loop.Tiled_Loop === "Not Tiled")) return null;

                        return (
                            <div key={index} className="loop-card">
                                <div className="loop-header">
                                    <h2>Loop {index + 1}</h2>
                                    <div className={`complexity-badge ${complexityColors[loop.Complexity_Class]}`}>
                                ...
                                <div className="loop-content">
                                    <div className="code-section original">
                                        <h3>Original Loop</h3>
                                        <SyntaxHighlighter language="cpp" style={vscDarkPlus}>{loop.Loop}</SyntaxHighlighter>
                                    </div>
                                    {loop.Parallelized_Loop !== "Not Parallelized" && (
                                        <div className="code-section parallelized">
                                            <h3>Parallelized Loop</h3>
                                            <SyntaxHighlighter language="cpp" style={vscDarkPlus}>{loop.Parallelized_Loop}</SyntaxHighlighter>
                                        </div>
                                    )}
                                    {loop.Tiled_Loop !== "Not Tiled" && (
                                        <div className="code-section tiled">
                                            <h3>Tiled Loop</h3>
                                            <SyntaxHighlighter language="cpp" style={vscDarkPlus}>{loop.Tiled_Loop}</SyntaxHighlighter>
                                        </div>
                                    )}

                                    <button className="configure-btn" onClick={() => replaceLoop(loop)}>
                ...
                <div className="completed-code-section">
                    <h2> Serial Code</h2>
                    <SyntaxHighlighter language="cpp" style={vscDarkPlus}>{codeInput}</SyntaxHighlighter>
                </div>
            </div>
            <Footer />
        </div>
